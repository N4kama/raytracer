...
obj_face* obj_parse_face(obj_growable_scene_data *scene)
{
    ...
    memset(face->vertex_index, 0, sizeof (face->vertex_index));
    memset(face->normal_index, 0, sizeof (face->normal_index));
    memset(face->texture_index, 0, sizeof (face->texture_index));

    int vertex_count = obj_parse_vertex_index(face->vertex_index,
            face->texture_index,
            face->normal_index);
...
{
    int temp_indices[MAX_VERTEX_COUNT] = { 0 };

    obj_sphere *obj = (obj_sphere*)malloc(sizeof(obj_sphere));
    memset(obj->texture_index, 0, sizeof(obj->texture_index));
    obj_parse_vertex_index(temp_indices, obj->texture_index, NULL);
    obj_convert_to_list_index_v(scene->vertex_texture_list.item_count,
            obj->texture_index);
    obj->pos_index = obj_convert_to_list_index(scene->vertex_list.item_count,
            temp_indices[0]);
    obj->up_normal_index =
        obj_convert_to_list_index(scene->vertex_normal_list.item_count,
                temp_indices[1]);
    obj->equator_normal_index =
        obj_convert_to_list_index(scene->vertex_normal_list.item_count,
                temp_indices[2]);
...
obj_plane* obj_parse_plane(obj_growable_scene_data *scene)
{
    ...
    obj_plane *obj = malloc(sizeof(obj_plane));
    memset(obj->texture_index, 0, sizeof(obj->texture_index));
    obj_parse_vertex_index(temp_indices, obj->texture_index, NULL);
    obj_convert_to_list_index_v(scene->vertex_texture_list.item_count,
            obj->texture_index);
...
{
    obj_light_point *o = malloc(sizeof(obj_light_point));
...
{
    obj_light_quad *o = malloc(sizeof(obj_light_quad));
...
vec3_t* obj_parse_vector()
{
    vec3_t *v = calloc(sizeof(*v), 1);
    const char *token = NULL;
    double *ptr = &v->x;

    /* it's valid for the Wavefront spec to only specify part on the vector */
...
void obj_parse_camera(obj_growable_scene_data *scene, obj_camera *camera)
{
    ...
    camera->camera_pos_index =
        obj_convert_to_list_index(scene->vertex_list.item_count,
                indices[0]);
    camera->camera_look_point_index =
        obj_convert_to_list_index(scene->vertex_list.item_count,
                indices[1]);
    camera->camera_up_norm_index =
        obj_convert_to_list_index(scene->vertex_normal_list.item_count,
                indices[2]);
}
    ...
    while (fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream))
    {
        ...
        else if (strequal(current_token, "map_Ka") && material_open)
        {
            strncpy(current_mtl->texture_filename, strtok(NULL, " \t"),
                    OBJ_FILENAME_LENGTH - 1);
            current_mtl->texture_filename[OBJ_FILENAME_LENGTH - 1] = 0;
        }
        else
        {
            fprintf(stderr, "%s '%s' in material file %s at line %i:\n\t%s\n",
                    "Unknown command", current_token, filename, line_number,
                    current_line);
        }
    }
...
int obj_parse_obj_file(obj_growable_scene_data *growable_data, char *filename)
{
    ...
    while (fgets(current_line, OBJ_LINE_SIZE, obj_file_stream))
    {
        ...
        {
            growable_data->camera = malloc(sizeof(obj_camera));
        ...
        else if (strequal(current_token, "mtllib")) // mtllib
        {
            strncpy(growable_data->material_filename, strtok(NULL, WHITESPACE),
                    OBJ_FILENAME_LENGTH - 1);
            growable_data->material_filename[OBJ_FILENAME_LENGTH - 1] = 0;
...
void obj_copy_to_out_storage(obj_scene_data *data_out, obj_growable_scene_data *growable_data)
{
    ...
    data_out->vertex_list = (vec3_t**) growable_data->vertex_list.items;
    data_out->vertex_normal_list =
        (vec3_t**) growable_data->vertex_normal_list.items;
    data_out->vertex_texture_list =
        (vec3_t**) growable_data->vertex_texture_list.items;
    ...
    data_out->material_list =
        (obj_material**) growable_data->material_list.items;
